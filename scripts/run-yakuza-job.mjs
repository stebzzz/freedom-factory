#!/usr/bin/env node
/**
 * run-yakuza-job.mjs — pipeline dedie storytime "vie d'un yakuza" (FR)
 *
 *   - Narration : script-videofr.txt (1er arg CLI aussi accepte)
 *   - Images    : WaveSpeed wan-2.7/image-edit + style-kit-yakuza.json
 *                 → coherence perso/style verrouillee par refs + seed.
 *                   Le perso EVOLUE en age (17→58) — le kit gere les marqueurs
 *                   (cheveux, costume, irezumi, auriculaire manquant).
 ...
 *
 * Usage : node scripts/run-yakuza-job.mjs [path/to/script.txt]
 */
...
const STYLE_KIT_PATH = process.env.STYLE_KIT
  ...
  : path.join(ROOT, "style-kit-yakuza.json");
...
// Priorite : environnement respire. Le Yakuza est petit / en retrait / partiel dans
// la plupart des cadrages — la ville et le systeme l'ecrasent. Un seul close-up
// autorise par scene pour un moment d'emphase (yubitsume, sakazuki, miroir metro).
...
documentaire-fiction sur LA VIE D'UN YAKUZA japonais, du recrutement (17 ans) jusqu'au sommet
(58 ans, kumicho). Setting moderne : Tokyo (Kabukichō, Shinjuku, Roppongi, Setagaya), Saitama,
Yokohama, Kobe. Annees 1990-2020. La narration est a la 2e personne ("Tu as 17 ans...").
...
- Palette: noir profond (costumes, kimono, asphalte), blanc os (chemises, visages, tabi),
  bleu nuit pluvieux, vermillon neon (lanternes, kanji), accents rose neon / cyan electrique
  reflets sur asphalte mouille (PAS sur les visages), or vieilli (boutons, montre, kamon),
  beige tatami, indigo, ambre whisky, vert-blanc fluorescent froid (dortoir / police), gris
  ardoise (Setagaya, prison).
- Outlines: bold black uniforme 3-5px, sur TOUT (persos, decors, costumes, props, kanji).
...
- Yakuza (perso central): identite LOCKED via style-kit, MAIS evolue en age (cheveux + costume +
  irezumi + auriculaire gauche). Voir REGLE D'AGE plus bas.

REGLE D'AGE (CRITIQUE — DOIT apparaitre dans imagePrompt):
Le script est decoupe en chapitres par age. Detecte l'age du protagoniste dans CHAQUE segment
en lisant le contexte narratif autour, puis ajoute dans imagePrompt une mention explicite type
"Yakuza at age 26, [outfit/markers specifiques a cet age]".

  - 17 ans (debut, Kabukichō recrutement, dortoir, premieres taches): cheveux noirs courts
    messy undercut + frange laterale, costume noir bon marche TROP GRAND aux epaules, chemise
    blanche, cravate fine noire de travers, chaussures noires usees. AUCUN tatouage. Les DEUX
    mains intactes (mitaines blanches normales). Posture courbee, soumise.
  - 21 ans (shatei, ceremonie sakazuki, premieres collectes): cheveux noirs slick-back propre,
    costume noir 2 boutons bien ajuste, chemise blanche, cravate noire unie, chaussures cirees.
    AUCUN tatouage. Mains intactes. Posture droite, serieuse.
  - 26 ans (kyodai, territoire Kabukichō, mikajimeryō, premier irezumi en cours): cheveux noirs
    pompadour slick-back, costume gris anthracite a fines rayures, cravate en soie cramoisie,
    pince a cravate doree, chaussures cuir noir. IREZUMI dragon noir/cramoisi/indigo qui depasse
    AU COL et aux POIGNETS comme petites volutes noires (le tatouage est sous le costume mais
    on voit son bord). Mains intactes. Posture confiante.
  - 32 ans (shateigashira, 8 hommes, yubitsume): cheveux noirs slick-back tendu cotes rasos,
    costume bleu marine 3 pieces, pochette burgundy, boutons de manchette dores, montre or au
    poignet droit. IREZUMI dragon visible au col et POIGNET DROIT (le poignet gauche aussi mais
    avec un BANDAGE BLANC pour les premieres scenes post-yubitsume). MAIN GAUCHE: silhouette
    de mitaine BLANCHE SANS L'AURICULAIRE (4 doigts seulement / petit moignon arrondi). Main
    droite intacte.
  - 39 ans (wakagashira, lieutenant, 80 hommes, paranoia): cheveux noirs slick-back, costume
    noir croise larges revers, chemise noire en soie sans cravate, chevaliere or massive. Irezumi
    plein col / poignets, croix sternale claire toujours preservee. Main gauche TOUJOURS sans
    auriculaire. Visage avec 2 petits traits diagonaux de fatigue aux coins des yeux.
  - 47 ans (saiko-komon, conseiller supreme, Setagaya): cheveux slick-back avec 2-3 meches
    grises aux tempes. Selon scene: kimono indigo + haori gris charbon avec KAMON noir circulaire
    sur poitrine et dos OU veste noire col mandarin moderne. Tabi blancs + geta si kimono. Main
    gauche sans auriculaire. Posture immobile, lasse.
  - 58 ans (kumicho, sommet, Kobe): cheveux full GRIS ACIER slick-back avec widow's peak. Kimono
    cérémoniel noir montsuki avec 5 KAMON blancs (poitrine, dos, manches), hakama gris, tabi
    blancs. Main gauche sans auriculaire. Posture seigneuriale, assise immobile.

Si segment EPILOGUE (retour sur un nouveau garcon de 17 ans devant le club mahjong sous la
pluie): c'est un AUTRE jeune yakuza (pas le protagoniste vieillissant), meme look 17-ans-debut.

AVANT D'ECRIRE LE JSON, raisonne sur l'arc narratif:
- Identifie age du protagoniste dans le segment + chapitre courant.
- Varie shot types: WIDE establishing neon-lit street, MEDIUM tatami room, CLOSE-UP visage du
  protagoniste, EXTREME CLOSE-UP insert (auriculaire coupe enveloppe dans tissu blanc, sake cup
  qui passe de main en main, dragon tattoo qui s'etire sur le bras dans onsen, lanterne rouge,
  flip-phone qui sonne), WIDE SYMBOLIC (skyline Tokyo nocturne, foule de salarymen).
- Decide quand le Yakuza est dans le cadre vs absent (env / metaphor / prop close-up).
...
{
  "environment": {
    "locationId": "string snake_case EN ANGLAIS (ex: kabukicho_alley_rain_night, mahjong_parlor_entrance, yakuza_dortoir_concrete, ramen_shop_shinjuku, oyabun_office_tatami, onsen_tattoo_session, prop_closeup_severed_finger)",
    "settingType": "string court (ex: 'narrow neon-lit Kabukichō alley', 'cramped concrete recruit dortoir', 'formal tatami room with paper shoji')",
    "timeOfDay": "morning / midday / afternoon / dusk / night / timeless",
    "weather": "clear / overcast / rainy / snowing / n/a",
    "lighting": "description specifique (ex: 'wet asphalt reflecting hot pink and cyan neon kanji signs from above, faces stay flat white', 'warm amber lantern light low on tatami, single red paper lantern glow', 'cold green-white fluorescent ceiling tubes', 'single hard interrogation spotlight from above')"
  },
  "visual": {
    ...
    "focus": "ce sur quoi le plan attire l'oeil (ex: 'the empty doorway of the mahjong club', 'the severed pinky wrapped in white cloth on the lacquer table', 'the dragon tattoo curling across the wet shoulder in the onsen steam')",
    "composition": "arrangement spatial (ex: 'Yakuza tiny lower-left dwarfed by vertical neon kanji wall on the right', 'symmetric tatami room, oyabun centered far back, protagonist kneeling foreground from behind', 'split frame: rain-streaked window left, neon street below right')",
    "sceneDescription": "DENSE 50-100 mots: decor + autres persos (oyabun, sempai, hôtesse, flic, rival, mère, frère, ex-femme, fille, etc.) avec traits visibles (cheveux/costume) + action concrete + props (lanterne, sake, batte, mahjong tile, briefcase, flip-phone, katana, white cloth) + lumiere. JAMAIS 'no characters present' (sauf prop-closeup pur). PAS de mention du Yakuza protagoniste ici (gere via character + generation.imagePrompt). PAS de mention du style (cartoon cel-shading — locked globalement)."
  },
  "character": {
    "present": boolean,
    "ageInScene": "integer entre 17 et 58 (l'age du protagoniste dans CE segment d'apres le contexte narratif). Si !present, mets l'age courant du chapitre quand meme.",
    ...
    "placement": "OU et COMMENT le Yakuza est dans le cadre. CRITIQUE: par DEFAUT il est PETIT, en retrait, partiellement visible — la ville/le systeme l'ecrasent. Ex: 'tiny silhouette under the doorway of the mahjong club, far back-center, drowned in neon' / 'kneeling foreground from behind, his back fills lower third, oyabun small far back across the tatami'. Centre large UNIQUEMENT pour emphase rare (yubitsume, sakazuki, miroir metro, mort). Vide si !present.",
    "pose": "posture corporelle (ex: 'shoulders hunched, head bowed' (17), 'still, hands clasped behind back' (32), 'seated seiza, hands on knees, motionless' (58)). Vide si !present."
  },
  "motion": {
    "environment": ["array de 1-3 mouvements: 'rain falls vertically and bounces on wet asphalt', 'neon signs flicker', 'cigarette smoke curls up from a glass ashtray', 'paper lantern sways gently', 'subway train passes left to right behind the window', 'steam rises from the onsen surface'"],
    "character": ["array de 0-2 micro-mouvements protagoniste: 'slight bow lower', 'left hand stump twitches once' (32+), 'eyes shift toward the door' — vide si !character.present"],
    "camera": {
      "type": "slow push-in / slow pull-back / lateral pan / static / subtle drift / nudge forward / slow tilt down / slow tilt up",
      "direction": "vers quoi (ex: 'toward the wrapped finger', 'down the neon alley corridor', 'up the kobe compound wall') — vide si static",
      "speed": "very slow / gentle / moderate / none-static"
    }
  },
  "generation": {
    "imagePrompt": "STRING dense directement injectable dans l'API image (wan-2.7) en ANGLAIS: reprends visual.sceneDescription + environment.lighting + visual.composition + (si character.present) une mention EXPLICITE de l'age du Yakuza et de ses marqueurs visuels pour cet age (cheveux, costume, irezumi visible/non, etat main gauche). Exemples : 'The Yakuza at age 17, oversized cheap matte-black suit too big at the shoulders, no tattoo visible, both hands intact, standing tiny under the doorway' / 'The Yakuza at age 32, dark navy three-piece suit, dragon tattoo curls peeking at his collar and right wrist, LEFT HAND mitten silhouette CLEARLY missing the little finger (4 digits only)' / 'The Yakuza at age 58, formal black silk montsuki kimono with white kamon crests at chest and sleeves, fully steel-grey slick-back hair, missing left pinky'. NE PAS ajouter le style (cartoon cel-shading — injecte separement). Vise 80-140 mots concatenes en un prompt fluide.",
    "negativePrompt": "elements a bannir pour CETTE scene (ex: 'realistic faces, photorealistic, anime style, modern hip-hop streetwear, intact left pinky on the protagonist, dragon tattoo on a child, European setting, samurai feudal Japan, blood splatter, gore')"
  }
}

REGLES GLOBALES:
- character.present = FALSE pour : gros plans props purs (severed pinky, sake cup, mahjong tile,
  flip-phone, lanterne), metaphores symboliques (skyline Tokyo, foule de salarymen, train de nuit),
  etablissements larges vides (compound de Kobe vu de loin sans figure visible).
...
- ageInScene est OBLIGATOIRE meme si character.present=false (sert a contextualiser le decor :
  un tatami a 47 ans n'est pas le meme decor qu'un dortoir a 17 ans).
- IMAGE PROMPT DOIT mentionner explicitement l'age + marqueurs (auriculaire manquant a partir
  de 32 ans, irezumi visible/cache a partir de 26 ans). C'est ce qui pilote la coherence d'age.
...
- Si N scenes partagent un id, CHACUNE doit montrer un COIN DIFFERENT du lieu (ex: kabukicho_alley
  → scene A entree de club, B passage etroit entre 2 immeubles, C bord de trottoir, D escalier
  exterieur en metal). JAMAIS la meme compo deux fois.
- Pour les phrases METAPHORIQUES / abstraites ("tu n'as pas tremble", "tu vaux plus mort que
  ...
  prop closeup symbolique (mains lavees sous le robinet, montre qui tourne, file de fourmis
  travailleuses, wagon de metro vide, lanterne qui s'eteint).
- Varie moments de la journee (morning / noon / dusk / night) entre scenes du meme locationId.

Exemples d'ids utilisables:
  "kabukicho_alley_rain_night", "mahjong_parlor_entrance", "mahjong_parlor_interior",
  "yakuza_dortoir_concrete", "ramen_shop_shinjuku", "subway_metro_window_reflection",
  "saitama_laundry_mother", "saitama_apartment_mother", "oyabun_office_tatami",
  "private_tatami_ceremony_sakazuki", "onsen_tattoo_session", "tattoo_artist_studio",
  "roppongi_apartment_view", "roppongi_club_neon", "shinjuku_skyline_night",
  "construction_site_bidrigging", "shareholders_meeting_corporate", "ministerial_reception_hall",
  "setagaya_house_garden_gravel", "setagaya_house_interior", "kobe_compound_exterior",
  "kobe_compound_meditation_room", "police_interrogation_room", "fuchu_prison_cell",
  "hospital_room_under_guard", "ex_wife_apartment_fukuoka", "daughter_wedding_distant".
Prop closeups: "prop_closeup_severed_pinky_white_cloth", "prop_closeup_sake_sakazuki",
  "prop_closeup_mahjong_tiles", "prop_closeup_flip_phone_ringing", "prop_closeup_red_lantern",
  "prop_closeup_dragon_tattoo_back", "prop_closeup_baseball_bat_broken_glass".
...
function defaultAnnotation() {
  return {
    environment: {
      locationId: "kabukicho_alley_rain_night",
      settingType: "narrow neon-lit Kabukichō alley",
      timeOfDay: "night",
      weather: "rainy",
      lighting: "wet asphalt reflecting hot pink and cyan neon kanji from above, faces stay flat white",
    },
    visual: {
      shotType: "wide establishing",
      focus: "the empty rain-slick alley",
      composition: "vertical neon kanji walls dominating both sides, character tiny far back",
      sceneDescription: "rain-slick narrow Tokyo alley at night, vertical neon kanji signs stacked floor by floor on both sides, glowing red lanterns, a single salaryman walking past holding a black umbrella, wet reflective asphalt mirroring the neon, faint smoke from a yatai cart on the corner",
    },
    character: {
      present: true,
      ageInScene: 17,
      expression: "head bowed slightly, mouth flat, eyes lowered",
      placement: "tiny silhouette far back-center under a doorway, drowned in neon, partially hidden by the column of a building",
      pose: "shoulders slightly hunched, hands at sides",
    },
    motion: {
      environment: ["rain falls vertically and bounces on wet asphalt", "neon signs flicker subtly"],
      character: [],
      camera: { type: "subtle drift", direction: "down the alley corridor", speed: "very slow" },
    },
    generation: {
      imagePrompt: "rain-slick narrow Kabukichō alley at night, vertical neon kanji signs stacked on both sides glowing hot pink and cyan, glowing red lanterns above doorways, a single salaryman walking past with a black umbrella, wet reflective asphalt mirroring the neon, faint smoke from a yatai food cart, the Yakuza tiny far back-center under a doorway, drowned in neon",
      negativePrompt: "realistic faces, photorealistic, anime, blood, gore, text overlays, watermark, intact left pinky after age 32",
    },
  };
}
          ...
          try {
            ...
            if (t.scene.generation?.imagePrompt) {
              ...
              if (t.scene.hasMainCharacter && t.scene.characterPlacement) {
                sceneDesc += `. Yakuza placement: ${t.scene.characterPlacement}`;
              }
              ...
              if (t.scene.ageInScene) {
                sceneDesc += `. The Yakuza in this shot is ${t.scene.ageInScene} years old — apply the exact ${t.scene.ageInScene}-year-old outfit, hair, irezumi visibility and left-hand pinky state described in the identity card.`;
              }
            } else {
