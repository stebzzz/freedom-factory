#!/usr/bin/env node
// recover-genaipro-earth-anim.mjs — recover downloads après un crash DNS
// du script animate-genaipro-earth.mjs.
//
// Lit /tmp/animate-genaipro-earth.log pour récupérer le mapping {row → taskId}
// (lignes "[vid#N] task UUID"), GET /v2/veo/histories paginé, télécharge les
// rows completed manquantes dans CLIP_DIR, et liste celles encore pending/failed.
...
const LOG_PATH = process.env.LOG_PATH || "/tmp/animate-genaipro-earth.log";
...
function httpsRequest(method, urlPath, headers, body, retries = 3) {
  return new Promise((resolve, reject) => {
    const attempt = (n) => {
      ...
      req.on("error", (err) => {
        if (n > 0) {
          console.warn(`[retry] ${method} ${urlPath}: ${err.code || err.message} (${n} left)`);
          setTimeout(() => attempt(n - 1), 5000);
        } else reject(err);
      });
      req.setTimeout(180_000, () => { req.destroy(); if (n > 0) attempt(n - 1); else reject(new Error(`HTTP ${method} timeout`)); });
  ...
  return new Promise((resolve, reject) => {
    const attempt = (n) => {
      https.get(target, { headers: { "User-Agent": "FreedomFactory/1.0" } }, (res) => {
        ...
        if (n > 0) { console.warn(`[retry-dl] ${err.code || err.message}`); setTimeout(() => attempt(n - 1), 5000); }
        else reject(err);
      });
    };
    attempt(retries);
...
function parseLog(logPath) {
  const map = new Map(); // taskId -> row n
...
async function fetchHistoriesByTaskIds(token, taskIds) {
...
async function main() {
  ...
  const promptByN = new Map(prompts.map((p) => [p.n, p]));
  const manifest = loadManifest();

  const taskMap = parseLog(LOG_PATH); // taskId -> row n
  const haveClip = existingClipRows();
  const toCheck = [...taskMap.entries()].filter(([, n]) => !haveClip.has(n));
  console.log(`Recovery: ${toCheck.length} task(s) à vérifier (${[...taskMap.values()].filter((n) => !haveClip.has(n)).sort((a, b) => a - b).join(", ")})`);

  const histories = await fetchHistoriesByTaskIds(token, toCheck.map(([t]) => t));

  const downloaded = [], failedN = [], stuckN = [];
  for (const [taskId, n] of toCheck) {
    const r = histories.get(taskId);
    const row = promptByN.get(n) || { epoch: `row_${n}` };
    if (!r) { stuckN.push(n); continue; }
    if (r.status === "failed") { console.log(`FAIL row ${n}: ${r.error || ''}`); failedN.push(n); continue; }
    if (r.status !== "completed") { stuckN.push(n); continue; }
    const url = r.file_urls?.[0];
    if (!url) { console.log(`NO URL row ${n}`); failedN.push(n); continue; }
    try {
      const buf = await httpsDownload(url);
      const ext = extFromUrl(url, "mp4");
      const fname = `${String(n).padStart(2, "0")}_${slug(row.epoch)}.${ext}`;
      const fpath = path.join(CLIP_DIR, fname);
      await writeFile(fpath, buf);
      console.log(`OK row ${n} -> ${fname} (${(buf.length / 1024 / 1024).toFixed(1)} MB)`);
      const entry = manifest.entries.find((e) => e.n === n);
      if (entry) entry.video = { taskId, url, path: fpath };
      else manifest.entries.push({ n, epoch: row.epoch, video: { taskId, url, path: fpath } });
      saveManifest(manifest);
      downloaded.push(n);
    } catch (e) {
      console.log(`DL FAIL row ${n}: ${e.message}`);
      failedN.push(n);
    }
  }
